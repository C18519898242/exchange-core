...
*   **刷盘机制 (Flush Triggers)**:
    *   系统并**不是基于时间的**（比如每隔几秒刷一次），而是**基于事件和容量**的，以实现性能和数据安全之间的平衡。刷盘（将内存缓冲区的数据强制写入磁盘文件）主要由以下三个条件触发：
    1.  **[批次](#批次-batch)结束时 (End of Batch)**: 这是最主要的触发器。当 `RingBuffer` 的生产者（`ExchangeApi`）发送一个批次的最后一个命令时，会附带一个 `endOfBatch=true` 的标志。当 `DiskSerializationProcessor` 处理到这个命令时，它会**立即执行一次刷盘**。这确保了每个批次的数据都被完整地持久化，是保障数据一致性的关键。
    2.  **缓冲区即将写满时**: 这是一个安全阀机制。如果在批次结束前，写入的命令就已经快要填满内存中的 `journalWriteBuffer`（默认256KB），系统会强制刷盘，以防止缓冲区溢出。具体的触发阈值是 `缓冲区大小 - 256字节`，预留出足够空间写入下一条可能的最大命令。
    3.  **特定管理命令**: 当收到一些特殊的系统管理命令时，会强制刷盘，以确保操作的原子性。这些命令包括：
        *   `PERSIST_STATE_*`: 在拍快照之前，必须将之前的日志全部落盘。
        *   `RESET`: 系统重置时。
        *   `SHUTDOWN_SIGNAL`: 系统正常关闭时。
...
在本文档的上下文中，“批次”并非一个业务概念，而是源于 LMAX Disruptor 框架的一种核心工作模式，是实现高性能的关键。

*   **定义**: 一个“批次”是指由生产者（`ExchangeApi`）**一次性连续发布**到 `RingBuffer` 上的一组命令。
*   **工作原理**:
    1.  **生产者 (`ExchangeApi`)**: 当外部调用 `ExchangeApi` 的方法时，它作为生产者，会向 `RingBuffer`（一个环形队列）申请一个或多个连续的“空位”（`OrderCommand` 对象）。
    2.  **批量申请**: 为了效率，生产者可以一次性申请一大批连续的空位（例如，通过 `ringBuffer.next(N)`），然后将多个命令依次填入。
    3.  **批量发布**: 所有空位都填满后，生产者通过 `ringBuffer.publish(low, high)`，一次性地将这一整批命令“发布”出去，使其对所有消费者可见。
*   **`endOfBatch` 标志**: 消费者（如 `GroupingProcessor`、`DiskSerializationProcessor`）在处理 `RingBuffer` 上的命令时，可以从 Disruptor 框架获知当前处理的命令是否是这个批次的**最后一个**。这个信息就是 `endOfBatch` 标志。
*   **意义**: `DiskSerializationProcessor` 正是利用了这个 `endOfBatch` 标志作为其最主要的刷盘触发器，从而在不牺牲过多性能的前提下，保证了每个逻辑批次的数据都能被完整地持久化，实现了性能与数据安全的精妙平衡。
